...
durch den Wert $eps := \frac{B^{1-n}}{2}$ nach oben beschränkt.  

Der Wert $eps$ wird auch [Maschinengenauigkeit]{.cstrong} genannt. Es gilt damit:

::: {.footnotesize}
$$rd(x) = x + \epsilon \cdot x = (1 + \epsilon)\cdot x \text{ mit } |\epsilon| < eps$$
:::
...
***E*** ist richtig. Möchte man in R oder Python auf Gleichheit prüfen (abfragen), so sind zwei Gleichheitszeichen nötig: `==`, da ein Gleichheitszeichen (`=`) für Zuweisungen eingesetzt wird. Daher sind die Antworten mit `=` falsch.
...
- *Addition* / *Subtraktion*:
    - absoluter Fehler: $\displaystyle \left|(\tilde{x} \pm \tilde{y}) - (x \pm y)\right| =  \left|- (e_x \cdot x \pm e_y \cdot y)\right|$
    - relativer Fehler: $\displaystyle \left|\frac{(x \pm y) - (\tilde{x} \pm \tilde{y})}{x \pm y}\right| = \left|\frac{x}{x\pm y}\cdot e_x \pm \frac{y}{x\pmy}\cdot e_y\right|$
    
- *Multiplikation*:
    - absoluter Fehler: $\left| x \cdot y -\tilde{x} \cdot \tilde{y}\right| = \left|-xy \cdot (e_x+e_y+ e_x \cdot e_y)\right|$
    - relativer Fehler: $\displaystyle\left| \frac{x \cdot y- (\tilde{x} \cdot \tilde{y})}{x \cdot y}\right| = e_x + e_y + e_x \cdot e_y$
...
Dabei sei $\tilde{x}$ der (ein) Eingabewert, welcher bei rundungsfreier Rechnung zu $\tilde{y}=f(\tilde{x})$ führt. (Hierbei interpretieren wir Rundungsfehler als Daten(eingabe)fehler.)
