...
Was passiert nun, wenn wir $x+y+z$ berechnen wollen? -- Wir haben dazu zwei Möglichkeiten:

Entweder a) $(x+y)+z$ oder b) $x+(y+z)$!
...
c) Da $f'$ monoton fallend auf $I=[`r x_tilde`; `r x`]$ gilt $M = \max\limits_{x_0 \in I} |f'(x_0)| = `r M`$ und somit:
   ...
   \left| \frac{f(x) - f(\tilde{x})}{f(x)} \right| &\leq M \cdot \frac{\max_{x_0 \in I}|x_0|}{\min_{x_0 \in I}{|f(x_0)|}} \cdot |x - \tilde{x}| \\ 
...
d) $K = M \cdot \frac{\max_{x_0 \in I}|x_0|}{\min_{x_0 \in I}{|f(x_0})|}} \cdot |x| = `r M` \cdot `r abs(max(x, x_tilde))/abs(min(f(x),f(x_tilde)))` \cdot `r abs(x)` = `r M  * abs(max(x, x_tilde))/abs(min(f(x),f(x_tilde))) * abs(x)`$
    Und damit:
