...
Verschiedene Werte von $B$ sind möglich bzw. werden verwendet, so etwa
...
Wählen wir in unserem Beispiel einen *Bias*wert von $2$, dann haben wir mit $(0,0)=0-2=-2$, $(0,1)=1-2=-1$, $(1,0)=2-2=0$ und $(1,1)=3-2=1$ die möglichen Exponenten (in 2 Bits kodiert).
...
B) Runden Sie die Zahlen $x_1$ bis $x_4$ auf eine 5-stellige Gleitpunktzahl und bestimmen Sie den Wert der Rundung.

C) Runden Sie die Zahlen $x_1$ bis $x_4$ auf eine 4-stellige Gleitpunktzahl und bestimmen Sie den Wert der Rundung.
...
**ABER:** Der Rechner hat so seine Probleme mit der *Gleichheit* von Zahlen. 

*Daher:* Wie würden Sie diese Gleichheit in einem Programm sinnvoll abfragen:
...
- Jede einzelne Operation ($+$, $-$, $\cdot$, $\ldots$) wird auf $n+1$ Stellen exakt gerechnet.
- Im Anschluss nach dieser Operation wird dann auf Grund der $n+1$-ten Stelle auf die $n$-te Stelle gerundet. 
- Erst im Anschluss wird die nächste Operation durchgeführt
...
\begin{aligned}
        x+(y+z)  &= (0.159)_{10} \cdot 10^4 + [(0.400)_{10} \cdot 10^1 + (0.400)_{10} \cdot 10^1] \\
                 &= (0.159)_{10} \cdot 10^4 + 0.8\cdot 10^1 \\
                 &= (0.159)_{10} \cdot 10^4 + (0.0008)_{10}\cdot 10^4 \\
                 &= (0.1598)_{10} \cdot 10^4 \qquad \text{(Runden und Normalisieren)} \\
...
- **ACHTUNG**: U.a. das Assoziativgesetz der Additivität gilt nicht mehr!
...
$$
    \begin{aligned}
        rd(\tilde{x}_1 -_M \tilde{x}_2)   &= (0.123454)_{10} \cdot 10^{12} - (0.123446)_{10} \cdot 10^{12} \qquad\text{(Runden und Normalisieren)}\\ 
...
Bei der Subtraktion zweier fast gleichgroßer Zahlen kann es zu einem besonderen Phänomen kommen: 
der **Auslöschung**.
...
Es soll das Fehlerfortpflanzungsverhalten der Funktion $f(x) = x^2 - 100$ an der Stelle $x=11.1$, $\tilde{x} = 11$, $I=[10.5, 11.5]$ untersucht werden:
