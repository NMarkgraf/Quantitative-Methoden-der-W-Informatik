...
### Python-Code für das Bisektionsverfahren {.shrink include-only=use_python}
...
### Beispiel: Bisektionsverfahren (I / II) {include-only=use_python}
...
```{python, eval=use_python, include=use_python, R.options = list(width = 400), echo=FALSE}
def print_iter_info(i, a, b, c, f):
...
```{python eval=FALSE, do_bisection_1, results="asis"}
...
```{r, out.width="60%", eval=use_python, include=use_python, echo=FALSE}
fkt <- makeFun(sqrt( (x-2)^2 + 1) - 2*(x+1)^2 ~ x)
p <- plotFun(fkt, x.lim=range(-0.5,1.5), h=0)
ladd(panel.abline(h=0,v=0,col='gray50'), plot=p)
```
...
```{python, eval=use_python, include=use_python, echo=FALSE}
def print_iter_info(i, x, fx, dfx):
    print(f'Iter. {i}: x={x:.8F} f(x)={fx:.8F} f\'(x)={dfx:.8F}\n')
```
...
Wo liegt, ausgehend von $x_1=2$, die Break-Even ($x_{BE}$) Gewinnschwelle?  -- Geben Sie eine Näherung mit Hilfe des Newtonverfahrens ($epsilon=0.00001$) an! Erstellen Sie dafür eine Tabelle mit $i, x_i, f(x_i), f'(x_i)$.

::: {.notes}

Wir erhalten für die Gewinnfunktion den Ausdruck $G(x)= -2 x^3 + 24x^2 -34 x - 60$ und für die Grenzgewinnfunktion $G'(x)= -6 x^2 +48 x - 34$, der Startwert $x_1 = 4$.
...
Möchte man die Bestimmung und Auswertung der Ableitung in jedem Schritt umgehen, weil z.B. die Ableitung schwer zu bestimmen ist, dann kann man die Tangentensteigung duch die Sekantensteigung der letzen zwei Punkte ersetzen.
...
### Python-Code Sekantenverfahren {include-only=use_python}
...
Wo liegt, ausgehend von $x_0=2$ und $x_1=4$, die Break-Even ($x_{BE}$) Gewinnschwelle?  -- Geben Sie eine Näherung mit Hilfe des Sekantenverfahrens ($epsilon=0.00001$) an! Erstellen Sie dafür eine Tabelle mit $i, x_i, f(x_i), \frac{\Delta f(x_i)}{\Delta x_i}$.
...
a) Bestimmen Sie den Break-Even-Point näherungsweise mit Hilfe des Sekantenverfahrens. Stellen Sie dazu die Tabelle mit den Werten $i, x_i, f(x_i), \frac{\Delta f(x_i)}{\Delta x_i}$ auf und rechnen Sie mit einer Genauigkeit von $epsilon = 0.00001$. Startwerte $x_0=1$ und $x_1=3$

b) Bestimmen Sie den Break-Even-Point näherungsweise mit Hilfe des Newtonverfahrens. Stellen Sie dazu die Tabelle mit den Werten $i, x_i, f(x_i), f'(x_i)$ auf und rechnen Sie mit einer Genauigkeit von $epsilon = 0.00001$. Startwert: $x_1=3$
...
### Beispiel: Regular-Falsi (I / II) {include-only=use_python}
...
### Beispiel: Regular-Falsi (II/ II) {.shrink inlcude-only=use_python}

```{python, results = "asis", eval=use_python, include=use_python, echo=FALSE}
def print_iter_info(i, a, b, c, f):
...
```{python, results = "asis", eval=FALSE, include=use_python}
if __name__== "__main__":
    print(regularfalsi(fkt, 0, 1, epsilon=0.000001))
```
\begin{verbatim}
```{python, results = "asis", eval=use_python, R.options = list(width = 400), echo=FALSE}
if __name__== "__main__":
    print(regularfalsi(fkt, 0, 1, epsilon=0.000001))
```
\end{verbatim}
